...
pub fn ReportHtmlHost(
    ...
    let chart_handles = Rc::new(RefCell::new(Vec::<
        leptos::mount::UnmountHandle<AnyViewState>,
    >::new()));
    ...
    Effect::new({
        ...
        move |_| {
            ...
            let Ok(node_list) = host_el.query_selector_all(".dice-output-chart[data-dice-output]")
            else {
                return;
            };
            ...
            for i in 0..node_list.length() {
                ...
                let handle = mount_to(
                    placeholder,
                    move || view! { <OutputEntryChart entry=entry.clone() /> },
                );
